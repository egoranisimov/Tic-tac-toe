...
result = False  # Переменная для остановки цикла в случае победы или ничьи
...
def check_result( user, marker ):
    global result
    if any([cells['1_1'] == cells['1_2'] == cells['1_3'] == marker,
            cells['2_1'] == cells['2_2'] == cells['2_3'] == marker,
            cells['3_1'] == cells['3_2'] == cells['3_3'] == marker,
            cells['1_1'] == cells['2_1'] == cells['3_1'] == marker,
            cells['1_2'] == cells['2_2'] == cells['3_2'] == marker,
            cells['1_3'] == cells['2_3'] == cells['3_3'] == marker,
            cells['1_1'] == cells['2_2'] == cells['3_3'] == marker,
            cells['3_1'] == cells['2_2'] == cells['1_3'] == marker]):
        print_field()
...
def next_step( current_user ):
    print_field()
    step = input(f'{current_user}, введи координату клетки: ')
    ...
    while True:
        if step not in cells.keys():
            step = input(f"{current_user}, координата клетки введена неправильно.\n"
            ...
            step = input(f"{current_user}, данная клетка уже использована, попробуй еще раз: ")
        else:
            used_cells.append(step)
            break

    # Цикл с присвоением соответствующего маркера соответстующей клетке и проверкой результата игры
    for i in cells.keys():
        if current_user == user_1:
            if i == step:
                cells[i] = 'O'
                check_result(current_user, 'O')
                break
        else:
            if i == step:
                cells[i] = 'X'
                check_result(current_user, 'X')
                break


# Приветственное сообщение, считывание имен пользователей, общая информация
